...
def merge_graph_into_result(ws, result_path: str) -> int:
    """把会话内存图谱里 result.json 没有的论文/节点/引用边增量合并回盘（只增不覆盖）。
    补跑的 ideation 会话禁用 emit_result 防覆盖正式结果，但 search_recent 新搜到的论文
    不该随内存一起丢——会话收尾调这个。返回新增论文数。"""
    ...
    fresh = ws.to_graph()
    added = 0
    for pid, p in fresh.papers.items():
        if pid in known:
            continue
        g["papers"][pid] = p.model_dump()
        known.add(pid)
        added += 1
    node_ids = {n["paper_id"] for n in g["nodes"]}
    for n in fresh.nodes:
        if n.paper_id not in node_ids:
            g["nodes"].append(n.model_dump())
            node_ids.add(n.paper_id)
    edge_keys = {(e["source"], e["target"]) for e in g["edges"]}
    for e in fresh.edges:
        if (e.source, e.target) not in edge_keys and e.source in known and e.target in known:
            g["edges"].append(e.model_dump())
...
def to_markdown(result: PipelineResult, path: str) -> None:
    _ensure_parent(path)
    r = result.report
    # report.md 此前只拼英文主字段，i18n.zh 翻译只有 view.html 在用——交付给用户的 md 成了
    # “中文标题+英文正文”的多宽体（实测整篇 0 个中文句号）。这里改为中文优先、
    # 逐字段回落英文，与前端同一策略。
    zh = result.i18n.get("zh") or {}

    def loc(field: str, default):
        v = zh.get(field)
    ...
    zh_stages = {
        s.get("paper_id_anchor"): s
        for s in (zh.get("stages") or [])
    ...
    if r.tldr:
        ...
        lines.append(loc("tldr", r.tldr) + "\n")
    if r.core_idea:
        lines.append(f"**核心思想**：{loc('core_idea', r.core_idea)}\n")
    if r.prerequisites:
        lines.append("### 前置知识")
        for x in loc("prerequisites", list(r.prerequisites)):
            lines.append(f"- {x}")
        lines.append("")
    if r.getting_started:
        lines.append("### 🚀 如何入手")
        for i, x in enumerate(loc("getting_started", list(r.getting_started)), 1):
            lines.append(f"{i}. {x}")
        lines.append("")
    if r.glossary:
        lines.append("### 📖 关键术语")
        for term, defn in loc("glossary", dict(r.glossary)).items():
    ...
    if r.seed_paper and r.seed_paper.paper_id and (
        ...
        if sp.summary:
            lines.append(f"- **论文简介**：{zh_sp.get('summary') or sp.summary}")
        if sp.relation_to_main_line:
            lines.append(f"- **与主干的关系**：{zh_sp.get('relation_to_main_line') or sp.relation_to_main_line}")
        lines.append("")
    ...
    if r.narrative:
        lines.append("## 📖 领域综述")
        lines.append(loc("narrative", r.narrative) + "\n")

    if r.stages:
        lines.append("## 🪜 发展阶段")
        for idx, st in enumerate(r.stages):
            z = zh_stages.get(f"stage_{idx}") or {}
            head = f"### {z.get('name') or st.name}" + (f"（{st.period}）" if st.period else "")
            lines.append(head)
            if st.summary:
                lines.append(z.get("summary") or st.summary)
...
def export_all(result: PipelineResult, out_dir: str, *, lite: bool = False) -> dict[str, str]:
    """lite=True: ideation 场景只需要 result.json（图谱本体）——报纸 HTML/PNG/GraphML/
    markdown 报告这些展示向产物全部跳过，省掉渲染开销。"""
