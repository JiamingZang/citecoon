...
class OpenAlexSource:
    ...
    def __init__(self, settings: Settings, cache: Cache | None = None):
        ...
        # 2026 起 OpenAlex 改每日预算制（免费 $0.1/天，UTC 午夜重置）：额度烧光后
        # 所有请求全 429——若仍 3 连重试会每次白烧 ~13s 还伪装成空结果（实测）。
        # 同 arXiv 熔断设计：计数器透传 + 预算耗尽后快败
        self.throttle_hits = 0
        self._budget_exhausted_until = 0.0

    def budget_exhausted(self) -> bool:
        import time as _t
        return _t.time() < self._budget_exhausted_until

    # ---- low level -------------------------------------------------------
    async def _get(self, path: str, params: dict | None = None) -> dict | None:
        import time as _t
        if _t.time() < self._budget_exhausted_until:
            self.throttle_hits += 1
            return None  # 预算窗内快败：缓存仍可用，发请求必 429 纯浪费时间
            ...
            for attempt in range(3):
                try:
                    ...
                    if r.status_code == 429:
                        ...
                        if retry_after > 120:
                            # 预算耗尽（retry-after 指向 UTC 午夜）：重试无意义，熔断至重置
                            self.throttle_hits += 1
                            self._budget_exhausted_until = _t.time() + min(retry_after, 3600)
                            return None
                        await asyncio.sleep(2 * (attempt + 1))
                        continue
                    r.raise_for_status()
                    data = r.json()
                    # 预算拒绝有时是 HTTP 200 + error body（非 429，实测）——不检测会
                    # 退化回静默空结果
                    if isinstance(data, dict) and "error" in data and "budget" in str(data.get("message", "")).lower():
                        self.throttle_hits += 1
                        self._budget_exhausted_until = _t.time() + 3600
                        return None
                    return data
    ...
    def _parse(w: dict) -> Paper:
        ...
        # PDF: scan ALL locations (primary + every alt), not just primary — conference
        # papers (CVPR/NeurIPS/...) often have no PDF at primary_location but an
        # open-access PDF elsewhere (e.g. CVF). This is what lets non-arXiv papers
        # still get full text instead of silently dropping out.
        pdf_url = None
    ...
    async def search(self, query: str, limit: int = 10) -> list[Paper]:
        ...
        if self.cache and papers:  # 空结果不缓存：防预算耗尽/限流期的假零命中被固化
