...
class ArxivSource:
    ...
    def __init__(self, settings=None, cache=None):
        ...
        # 同 OpenAlex 一套 sqlite 缓存：命中即零请求零排队——arXiv 官方讨论组点名的
        # 最佳实践（重复查询应缓存到本地而非重打 API），也是限流下的主要退路
        self.cache = cache
        # arXiv asks for <= 1 request / 3s; be gentle (~1 / 3.3s) to avoid 503s.
        self._limiter = RateLimiter(rate_per_sec=0.3)
        # 限流/网络异常计数器：调用方比较前后差值即知本轮空结果是否可疑——
        # 此前限流被伪装成 "no papers found"，误导 agent 一轮轮换词白烧（实测）
        self.throttle_hits = 0
        # 熔断窗：确认被限流后的一段时间内所有请求直接快败（不排队不重试）——
        # 否则限流窗口里每次调用仍要烧 limiter 3.3s + 重试睡 4s，一次 find_candidates
        # 四路叠加实测白等 65s；快败后 OpenAlex 仍在干活，窗口过后自动恢复
        self._cooldown_until = 0.0

    def cooldown_remaining(self) -> int:
        import time as _t
        return max(0, int(self._cooldown_until - _t.time()))
    ...
    async def _fetch(self, url: str, params: dict | None = None,
        ...
        import time as _t
        if _t.time() < self._cooldown_until:
            self.throttle_hits += 1
            return None  # 熔断窗内直接快败，不排队不发请求
            ...
            try:
                ...
                if r.status_code in (503, 429):
                    if attempt == 0:
                        ...
                        continue
                    self.throttle_hits += 1
                    self._cooldown_until = _t.time() + 25
                    return None
                r.raise_for_status()
                return r
    ...
    async def search(
        ...
        """
        q = query.strip()
        ck = f"axv:search:{q}:{max_results}:{sort}"
        if self.cache and (c := self.cache.get(ck)) is not None:
            return [Paper(**p) for p in c]
        if re.search(r"\b(?:ti|abs|au|cat|all):", q):
            # 已结构化查询（带字段前缀/布尔链，如 ti:"LLaDA2.0" OR …）直通：
            # 再包一层引号+all: 会把它变成废查询（实测变体 OR 链被包后零命中）
            search_query = q
        else:
        ...
        if r is None:
            return []
        papers = self._parse_feed(r.text)
        if self.cache and papers:  # 空结果不缓存：防止限流期的假零命中被固化
            self.cache.set(ck, [p.model_dump() for p in papers])
        return papers

    async def get_by_id(self, arxiv_id: str) -> Paper | None:
        """按 id 直查单篇（id_list 接口）。验真用：超新论文 OpenAlex 常未索引，
        只靠它会把真 id 误判成幻觉（实测 LLaDA 2502.09992 被标 ✗）。"""
        aid = re.sub(r"^arxiv:", "", arxiv_id.strip(), flags=re.IGNORECASE)
        ck = f"axv:id:{aid}"
        if self.cache and (c := self.cache.get(ck)) is not None:
            return Paper(**c)
        r = await self._fetch(_API, params={"id_list": aid, "max_results": 1})
        if r is None:
            return None
        papers = self._parse_feed(r.text)
        if self.cache and papers:
            self.cache.set(ck, papers[0].model_dump())
