...
多人绝对3D位姿估计要求从单目RGB图像/视频中同时恢复场景中每个人的完整三维关节坐标（相机坐标系），其核心难点在于**绝对根深度**的确定——即每人距相机的真实距离。Moon等人（2019）提出的3DMPPE框架（DetectNet→RootNet→PoseNet）以面积比距离度量 $k = \sqrt{f_x \cdot f_y \cdot A_{\text{real}} / A_{\text{img}}}$ 估计根深度 $Z_R$，成为该任务的标准top-down管线。然而该管线存在两个结构性瓶颈：

**瓶颈一：根深度在遮挡/截断下崩溃。** RootNet的距离度量依赖成像面积 $A_{\text{img}}$ 的准确估计。当人体被他人/物体遮挡或画面截断时，检测框面积严重偏离真实投影面积，导致深度估计失效。Moon等人（2019）原文图4已给出反例。Root-GAST-Net（2022）在Human3.6M上的实验表明，RootNet原版的MRPE达289 mm，而其改进版仍为178 mm——根深度误差在绝对位姿总误差中占比超过50%。
...
| 瓶颈 | 量化指标 | 来源 |
|------|---------|------|
| 根深度遮挡失效 | MRPE 289 mm（Human3.6M） | Root-GAST-Net卡 |
...
| 方法 | 年份 | 关键指标 | 根深度策略 | 遮挡处理 |
|------|------|---------|-----------|---------|
| 3DMPPE (Moon) | 2019 | MRPE 289 mm (H36M) | 面积比 $k$ | 无 |
| SMAP | 2020 | RtError 23.3 cm | 根深度图（稀疏） | 深度排序遮挡优先 |
| HMOR | 2020 | PCKabs +12.3 (MuPoTS-3D) | 层次序关系 | 训练时弱监督 |
| Root-GAST-Net | 2022 | MRPE 178 mm (H36M), 3DPCKabs 56.8% (MuPoTS-3D) | 面积比（同RootNet） | 时序lifting 27帧 |
| **本方案** | — | 目标MRPE≤150 mm (H36M) | 面积比+SMPL修正+序数损失 | GDN门控时序记忆 |
...
**第一步：遮挡使面积信号退化。** RootNet的距离度量 $k = \sqrt{f_x f_y A_{\text{real}} / A_{\text{img}}}$ 中，$A_{\text{img}}$ 由检测框面积近似（`data/Human36M/Human36M.py:122`：`area = bbox[2]*bbox[3]`）。遮挡/截断时检测框仅覆盖可见部分，$A_{\text{img}}$ 系统性偏小→$k$偏大→$Z_R$偏小（人被判为更近）。这不是随机噪声而是**方向性系统偏差**，无法通过数据增强或更大backbone消除。
...
- **RootNet**：ResNet-50/152 backbone，输入单人crop图像，分别预测根关节2D像素坐标 $(u_R, v_R)$ 和绝对深度 $Z_R$。深度通过距离度量 $k = \sqrt{f_x \cdot f_y \cdot A_{\text{real}} / A_{\text{img}}}$ 学习（`data/dataset.py:73`），其中 $f_x, f_y$ 为相机焦距（像素单位），$A_{\text{real}}$ 为人体真实面积（H36M 中 `bbox_real=(2000,2000)` mm，`config.py:38`）。三模块采用分离式训练（非联合），因"两任务相关性不高，分离训练精度更高"（camera_distance_aware卡）；
...
RootNet根深度估计的核心公式（Moon等，2019；`main/model.py:67-72`）：

$$Z_R = \gamma_{\text{pred}} \cdot k = \gamma_{\text{pred}} \cdot \sqrt{\frac{f_x \cdot f_y \cdot A_{\text{real}}}{A_{\text{img}}}}$$

网络输出头结构（`main/model.py:67-72`）：backbone最终特征（2048×8×8）经全局平均池化→2048×1×1→1×1 Conv（`self.depth_layer`，`model.py:22-28`，in=2048, out=1）→标量 $\gamma_{\text{pred}}$→乘 $k$ 得绝对深度。由针孔模型 $A_{\text{img}} \approx f_x f_y A_{\text{real}} / Z^2$ 可推导 $k \approx Z$（相机到人体的距离），因此 $\gamma_{\text{pred}}$ 为无量纲比例修正因子，理想值为1。
...
**可回退设计.** 本贡献为纯工程集成，不修改任何模型结构。若后续Contribution 2/3的改进模块失效，可直接回退到本基线的逐帧推理模式，性能下界即为3DMPPE原版精度（MRPE 289 mm (H36M) / 3DPCKabs约44%，camera_distance_aware论文Table 2）。
...
**代码锚点（3DMPPE_ROOTNET_RELEASE repo卡）：** 原始 $A_{\text{img}}$ 的计算位于 `data/Human36M/Human36M.py:122`（`area = bbox[2]*bbox[3]`），随后在 `data/dataset.py:73` 代入 k_value 公式（`k_value = sqrt(bbox_real[0]*bbox_real[1]*f[0]*f[1]/area)`）。深度输出头（`main/model.py:67-72`）为 GAP(2048×8×8)→Conv1×1(2048→1)→标量 $\gamma$→`depth = gamma * k_value`。SMPL面积修正的注入点为 `data/dataset.py:73` 的 `area` 变量：将 `area` 替换为 $A_{\text{eff}}$（见下式），即可在不改动网络结构的前提下修正 k_value。
...
**接口约定：** SMPL面积修正模块以plugin形式接入RootNet的面积计算环节（注入点：`data/dataset.py:73` 的 `area` 变量，参见repo卡改造点1），不改变RootNet的网络结构（原仓库无任何SMPL相关代码或依赖，需从外部引入smplx库）。当 $w_A > 0.9$ 时修正量 $< 1\%$，等价于无修正（加法式改造）。

**(2b) 序数深度监督.** 借鉴Pavlakos等（2018）的序数深度思想，为RootNet提供不依赖面积绝对精度的相对深度约束。

**代码锚点（3DMPPE_ROOTNET_RELEASE repo卡）：** 当前RootNet损失为纯L1（`main/model.py:106-113`：`loss_coord = torch.abs(coord - target_coord) * target_vis`，xyz三通道等权平均），无序数深度监督、无面积修正项。2D数据集通过 `target_have_depth=0` 屏蔽z轴损失。序数损失的挂载点为 `model.py:111-112` 的 `loss_coord` 计算之后（repo卡改造点3）：`return loss_coord + lambda_ord * loss_ordinal`。`train.py:49` 的 target dict 需扩充以传入多人同帧的GT深度序标签。
...
**与现有系统的衔接.** 序数深度监督仅在训练时生效（多人合成数据MuCo-3DHP提供多人同帧GT），推理时不增加计算。SMPL面积修正的MLP头参数量约0.13M，推理额外开销可忽略。评估阶段的投影反算位于 `data/Human36M/Human36M.py:155-160`：先将heatmap坐标映射回原图像素空间，再调用 `pixel2cam`（`common/utils/pose_utils.py:13-18`：$X = (u - c_x) \cdot Z / f_x$, $Y = (v - c_y) \cdot Z / f_y$）完成针孔模型反投影。SMPL面积修正仅改变 k_value 中的面积项，不影响此反投影流程。
...
| MRPE (Human3.6M) | 平均根位置误差 | 289 mm (RootNet) / 178 mm (Root-GAST-Net) | ≤150 mm | ≥15%↓ vs Root-GAST-Net |
...
- MuPoTS-3D：20段室内外多人视频（测试），最多3人/场景
...
| W3-4 | C1: 3DMPPE复现 | RootNet+PoseNet训练完成；视频化跟踪集成 | H36M MRPE≤300 mm, MuPoTS-3D 3DPCKabs≥45% | **Go条件：** MRPE≤320 mm即可继续。若>350 mm→检查数据预处理/相机参数 |
...
| W13-14 | MuPoTS-3D全管线评估+论文撰写 | 完整结果表+可视化 | H36M MRPE≤150 mm, MuPoTS-3D 3DPCKabs≥62% | 若MRPE>200 mm→调整目标为"优于Root-GAST-Net"（H36M≤178 mm） |
...
本方案以3DMPPE（DetectNet→RootNet→PoseNet）为基线，通过三个递进贡献构建遮挡鲁棒的多人绝对3D位姿估计系统：(1) 复现基线并视频化扩展；(2) 以SMPL面积修正+序数深度监督加固RootNet根深度在遮挡/截断下的鲁棒性；(3) 以GDN门控delta规则线性注意力替换单帧PoseNet为时序lifter，核心创新为解耦观测质量门（双通道关断：置信度+几何一致性）与状态惊讶门，实现"遮挡期保护记忆、恢复期按需读写"。预期在Human3.6M上MRPE从289 mm降至≤150 mm，H36M MPJPE至少持平PoseMamba（38.1 mm）。主要风险为四仓库集成调试复杂度与GDN门控训练收敛性，均有结构性回退路径（加法式拆除，下界为3DMPPE原版）。时间框架16周（约4个月），单卡≤24 GB GPU预算，目标为本科毕业论文答辩。
