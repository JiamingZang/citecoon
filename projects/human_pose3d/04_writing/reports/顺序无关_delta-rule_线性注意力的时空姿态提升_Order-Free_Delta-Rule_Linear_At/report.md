...
> 技术可行性报告 · 2026-07-21 初版 · 2026-07-23 局部修订 · idea: 顺序无关delta-rule线性注意力的时空姿态提升.md · ReAct 写作（边写边查证 papers/cards/codebases）
...
**瓶颈二：效率—表达力的虚假二元对立。** 领域经历了膨胀卷积→Transformer→SSM 的三代架构竞赛（_themes.json 母题"自注意力二次复杂度…驱动了膨胀卷积→Transformer→SSM/Mamba 的三代架构替代竞赛"）。Transformer 的自注意力对 token 数呈 $O(N^2)$ 复杂度——PoseFormer 卡指出"将所有关节×帧作为 token 直接输入 Transformer 会导致 token 数爆炸（如 243×17=4131），计算复杂度不可接受"；SSM 换来 $O(N)$ 复杂度，却以顺序递推为代价牺牲了内容寻址能力。跨领域证据表明，固定大小递归态（Mamba/RWKV/H3）在需要精确召回的任务上系统性弱于注意力机制（BASED, arxiv:2402.18668，详见 §1.3 命题 2）。Test-time regression 统一框架（arxiv:2501.12352）进一步从原理层面证明：关联记忆本质是 test-time 回归，线性注意力与 SSM 均为其特例——空间维的关节 token 召回可被建模为对上下文的加权回归，无需依赖顺序递推（详见 §1.3 命题 3 补充）。
...
2023–2025 年语言模型序列架构已推进至门控线性注意力 / delta-rule 线性注意力，本工作直接借鉴以下四篇核心文献（均核对 papers/ 原文），并引用五篇补充文献：
...
- **RWKV-7**（arxiv:2503.14456, "Goose", 2025）：将 delta 规则推广为向量值门控形式——上下文学习率从标量扩展为向量（逐通道选择性状态替换），解耦移除键 $\kappa$ 与替换键 $\tilde{k}$，引入向量值状态门控（衰减 $w$）与替换率增强因子 $\alpha$，使转移矩阵 $G_t = \mathrm{diag}(w_t) - \hat{\kappa}_t^\top(a_t \odot \hat{\kappa}_t)$ 成为缩放 Householder 矩阵的近似，特征值稳定在 $[-1,1]$（RWKV-7 卡）。2.9B 模型在 3.1T token 上训练，多语言下游达 3B SoTA。本工作将其作为**可选增强变体**：以向量门控 + 解耦键替代 GDN 标量门控，获得更细粒度的逐维记忆管理（§3.2 消融 A13 对比）。
- **Titans**（arxiv:2501.00663, 2025）：将记忆训练视为在线学习问题，用关联记忆损失对输入的梯度衡量"惊讶度"（surprise），惊讶度高的事件更值得记忆；引入衰减机制管理有限记忆容量。其核心洞察——梯度大小作为记忆优先级代理——与本工作 delta 误差项 $\mathbf{v}_j - \mathbf{S}_{j-1}^\top \hat{\mathbf{k}}_j$ 的语义一致（误差大则写入多）。
- **Test-time regression**（arxiv:2501.12352, 2025）：将关联回忆形式化为"记忆 + 检索"两步过程，其中记忆步骤被转化为对输入 token 的加权回归。任何序列层由三个设计选择唯一确定：(1) 回归权重 $\gamma_i^{(t)}$，(2) 回归器函数类 $\mathcal{M}$，(3) 测试时优化算法。线性注意力、SSM、fast-weight、softmax attention 均为该框架的特例（Test-time regression 卡）。该框架从原理层面证实：空间维关节 token 的召回问题可被建模为对上下文的加权回归，无需依赖顺序递推（详见 §1.3 命题 3 补充）。
- **GDN µP**（arxiv:2606.04048, 2026）：首次给出 Gated Delta Network 的完整 µP（最大更新参数化）公式。关键发现：门控权重矩阵需非标准 $\Theta(1/\sqrt{d})$ 学习率缩放，标量门控参数需 $\Theta(\sqrt{d})$ 缩放；在 RMSNorm 前插入 $\sqrt{d}$ 乘子以保证输入为 $\Theta(1)$ 坐标尺度。AdamW 下最优学习率在 4 个模型宽度间一致（零样本学习率迁移），而标准参数化下最优学习率随宽度显著漂移（GDN µP 卡）。本工作参照其初始化与缩放规则（§2 超参数），但需注意该规则在 17–28 token 极短序列下的适用性存在不确定性。
- **Sapiens**（arxiv:2408.12569, ECCV 2024）：以 0.3B–2B 参数 ViT 在 3 亿张人体图像上 MAE 预训练，统一 2D 姿态（308 关键点）、部位分割、深度与法线估计。其"人体领域自监督预训练优于通用预训练"的结论（Sapiens 卡）为本工作未来整合大规模人体预训练特征提供了上游基础。

GDN 的官方开源实现为 NVlabs/GatedDeltaNet（GDN 卡 resources："代码开源: https://github.com/NVlabs/GatedDeltaNet"）；fla-org/flash-linear-attention 库提供了上述模块（含 GDN、GLA、DeltaNet）的高效 Triton 实现（详见 §2 与 codebases/flash-linear-attention.md）。RWKV-7 开源于 RWKV/RWKV-LM（Apache 2.0，RWKV-7 卡 resources）。这些方法保持 $O(N)$ 复杂度，但将信息存入 key→value 关联记忆、按内容寻址召回，召回不依赖序列位置——恰好对症姿态任务中空间维无序的痛点。
...
**理论统一性补充（Test-time regression 框架，arxiv:2501.12352）。** 上述 delta-rule-as-online-SGD 的解释并非孤立的类比，而是 Test-time regression 统一框架的特例。该框架证明：任何序列层的关联回忆均可形式化为"记忆 + 检索"两步过程，其中记忆步骤等价于对输入 token 的加权回归——线性注意力（均匀权重）、GLA/Mamba/RWKV-6（衰减权重）、DeltaNet/Gated DeltaNet（参数化回归 + SGD）、softmax attention（精确解 / 核回归）均为该框架在不同设计选择下的特例（Test-time regression 卡）。对姿态任务的直接推论：空间维 $J=17$ 个关节 token 的召回问题，本质上可被建模为对上下文的加权回归（选择 delta-rule SGD 作为测试时优化算法），无需依赖顺序递推——这从原理层面（而非仅凭直觉类比）支撑了"空间维用关联记忆替代顺序扫描"的设计选择。该框架同时指出"线性注意力因未建模 token 间相关性而表达力不足"（Test-time regression 卡 limitation），这恰好解释了为何本工作选择 delta-rule（含 key 碰撞处理）而非简单线性注意力（消融 A4 验证）。
...
**超参数设置（参照 PoseMamba-B，核对原文 Table 6）。** $d_m=128$，层数 $N=20$，注意力头 $H=4$，$d_k=d_v=64$，MLP 扩展比 2。PoseMamba-B 配置为 depth=20、dm=128、3.358 M 参数、13.9 G MACs（原文 `posemamba_*.md:378-382`）。训练：AdamW，lr=$2\times10^{-4}$，指数衰减 0.99/epoch，权重衰减 0.01，batch size 4，120 epochs，水平翻转增强（PoseMamba 卡 resources）。**初始化与学习率缩放（参照 GDN µP，arxiv:2606.04048）：** 门控权重矩阵（$\mathbf{w}_\alpha, \mathbf{w}_\beta$ 的投影）采用 $\Theta(1/\sqrt{d})$ 学习率缩放，标量门控参数（$b_\alpha, b_\beta$）采用 $\Theta(\sqrt{d})$ 缩放；在 RMSNorm 前插入 $\sqrt{d}$ 乘子保证输入为 $\Theta(1)$ 坐标尺度（GDN µP 卡）。该规则使 AdamW 下最优学习率在 4 个宽度间零样本迁移（GDN µP 卡实验：$d\in\{256,512,1024,1536\}$）。**适用性风险：** GDN µP 的推导假设循环状态稳态方差等于写入更新方差（几何级数收敛），要求 $\alpha_t, \beta_t$ 始终远离 0 和 1（GDN µP 卡 core_assumption）；该分析面向语言模型的数千 token 序列，在 $J=17$–$28$ 的极短空间序列上是否仍成立存在不确定性——若递推步数过少导致几何级数未收敛，µP 缩放可能不再最优。缓解：若 µP 初始化下训练不稳定，回退至标准参数化（Xavier 初始化 + 统一学习率），并在 W4 消融中对比两种参数化方案。
...
| A13 | 空间维: RWKV-7 向量门控 + 解耦键（替代 GDN 标量门控） | 对比标量 vs 向量门控的表达力：逐通道选择性替换是否优于全局标量衰减 | Ablation |

**Oracle 上界（A10）说明：** $J=17$ 时 $O(J^2)$ 自注意力开销可接受（$J^2=289$），其无位置编码版本提供"顺序无关 + 满秩交互"的性能天花板；若 A3（delta-rule）逼近 A10，则证明线性复杂度关联记忆足以替代二次注意力。**Negative control（A11/A12）说明：** A11 验证"破坏扫描顺序会损害 Mamba"（对照本方法对置换不敏感），A12 验证"时间维若去掉顺序递推会退化"（对照命题 4 的时空不对称性）。**A13 说明：** RWKV-7 的向量值门控允许逐通道选择性状态替换（GDN 标量门控对所有通道施加相同衰减率），且解耦移除键 $\kappa$ 与替换键 $\tilde{k}$ 使删除和写入操作独立（RWKV-7 卡）；若 A13 显著优于 A3，则表明 $J=17$ 的短序列已足以受益于更细粒度的门控表达力，反之则确认标量门控对极短序列已足够。
...
| 消融实验 (13组) | 1× RTX 3090 | ~5–7 天 (串行) | 可 2–3 组并行（若显存允许） |
...
| NVlabs/GatedDeltaNet | 低（参照） | GDN 官方开源实现（GDN 卡 resources）；本工作参照其模型结构与门控设计，但空间维使用自研 naive 递推而非其因果 Triton kernel | 仅作实现参照，不直接依赖其训练管线 |
| RWKV/RWKV-LM (Apache 2.0) | 低（可选） | RWKV-7 开源实现（RWKV-7 卡 resources）；仅在消融 A13 中需要复现向量门控变体 | 若 A13 非核心路径，可仅以公式自实现（~50 行） |
...
| GDN µP 缩放规则在 $J=17$–$28$ 极短序列下不适用 | 中 | 初始化/学习率不最优，收敛变慢或精度下降 | 回退至标准参数化（Xavier + 统一 lr）；W4 消融对比 µP vs SP 两种方案（GDN µP 卡 limitation：推导假设几何级数收敛，需 $\alpha_t, \beta_t$ 远离 0/1） |
...
| W5 | 完整消融 A7–A13 + 估计 2D 实验 | 完整消融表 + 主表 | 确认 oracle 上界与 negative control；A13 对比标量 vs 向量门控 |
...
- **工程可行性**：高。单卡 3090 可跑（PoseMamba 卡 resources），数据准备成熟（PoseMamba repo 卡路径），核心模块基于 fla naive 语义实现 ~200 行，**不依赖** fla 因果 Triton kernel，规避了 chunk_size 与因果 mask 约束。外部仓库涉及 GarrickZ2/PoseMamba、JinluZhang1126/MixSTE、NVlabs/GatedDeltaNet + fla-org/flash-linear-attention（均为参照，非强依赖）。新增风险点：GDN µP 缩放规则（arxiv:2606.04048）在 17–28 token 极短序列下是否仍适用存在不确定性，可能需回退至标准参数化（§2 超参数已给出缓解方案）。
...
| 公式 | RWKV-7 转移矩阵 $G_t = \mathrm{diag}(w_t) - \hat{\kappa}_t^\top(a_t \odot \hat{\kappa}_t)$，特征值 $[-1,1]$ | cards/rwkv_7_goose_*.json method 字段 |
| 框架 | Test-time regression：序列层 = (回归权重, 函数类, 优化算法)；线性注意力/SSM/softmax attention 均为特例 | cards/test_time_regression_*.json method 字段 |
| 训练 | GDN µP：门控权重 $\Theta(1/\sqrt{d})$ lr 缩放，标量门控 $\Theta(\sqrt{d})$ 缩放；AdamW 零样本 lr 迁移 | cards/unlocking_feature_learning_*.json method/eval_setup |
| 记忆 | Titans：惊讶度（关联记忆损失梯度）驱动记忆优先级，衰减 = momentum + weight decay | cards/titans_learning_to_memorize_*.json method 字段 |
| 预训练 | Sapiens：0.3B–2B ViT，3 亿人体图像 MAE 预训练，308 关键点 | cards/sapiens_foundation_*.json method/eval_setup |
| 代码 | NVlabs/GatedDeltaNet（GDN 官方实现）；RWKV/RWKV-LM（Apache 2.0） | cards/gated_delta_networks_*.json resources；cards/rwkv_7_*.json resources |
...
>
> 2026-07-23 局部更新：(1) 技术路线升级为 GDN（arxiv:2412.06464）精确门控 delta 规则公式，新增开源实现 NVlabs/GatedDeltaNet 参照，新增 RWKV-7（arxiv:2503.14456）向量门控变体作为可选增强与消融 A13；(2) Gap 来源与动机补充 Test-time regression（arxiv:2501.12352）统一框架作为理论支撑（§1.1 瓶颈二、§1.3 命题 3 补充）；(3) 消融矩阵新增 A13（GDN 标量门控 vs RWKV-7 向量门控），训练细节补充 GDN µP（arxiv:2606.04048）初始化与学习率缩放规则及其短序列适用性风险；(4) 相关论文新增 5 篇（RWKV-7、Titans、Test-time regression、GDN µP、Sapiens）；(5) 外部依赖风险表新增 NVlabs/GatedDeltaNet 与 RWKV/RWKV-LM，错误传播风险表新增 µP 短序列适用性风险。所有新增引用均已核对 cards/ 精读卡。
