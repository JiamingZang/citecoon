...
这些数字共同说明：现有 SOTA 在受控分布（Human3.6M 室内、S1/5/6/7/8→S9/11 划分）上把 MPJPE 压到 35–45 mm 量级，但**没有任何方法给出"这一次预测是否可信"的标量信号**。在康复步态分析、AR 虚拟试穿、机器人选择性预测等下游场景中，"知道不知道"比"再降 1 mm"更关键。
...
- **Anatomy-Aware (W3126541466, 2021)** — 将 3D 关节位置预测显式分解为骨骼方向与骨骼长度两个子任务，骨骼长度跨全视频随机采样 $l=50$ 帧聚合估计。其骨长恒定假设与运动学树分解为本报告所提"解剖学合理性重加权"贡献提供了直接借鉴。
...
### 1.2.3 不确定性路线（产出不确定性但不在多假设姿态上）

领域内现有的不确定性估计主要分布在医学影像、6D 物体姿态等任务（参见 idea 评审记录的查重 top 列表），3D 人体姿态上的"多假设分歧→校准不确定性"路线**目前没有直接对应工作**（idea 已通过 3/3 对抗评审，多数派未能驳倒）。最接近的信号是 DiffPose 卡中提到的"高不确定性和不确定性（indeterminacy）"动机，但 DiffPose 把不确定性用作扩散先验而非输出。
...
本方案拆解为三个互补贡献：(C1) 多假设逐关节分布保留——替换 MHFormer 的 1×1 Conv1d 回归头与 DiffPose 的均值算子，输出 K 条假设的逐关节高斯；(C2) 解剖学合理性重加权——借鉴 Anatomy-Aware 的骨长/运动学一致性对假设打分，避免方差被离群假设污染；(C3) 温度缩放 + isotonic 回归校准——用成熟 sklearn 组件把假设散布校准为可信的置信区间。下面逐一展开。
...
母题 4（`cards/_themes.json`）指出："骨长恒定、关节角限制、左右对称、运动学树被反复手工注入作为弱监督或正则化"。Anatomy-Aware 卡（W3126541466）的骨长一致性、Structure and Motion 卡的"非法角度损失（膝/肘不超过 180°）"、Weakly-Supervised 卡的"骨骼长度比例不变性 3D 几何约束"都提供了可直接复用的解剖学约束。

### 2.2.2 技术细节：骨长一致性打分

定义骨骼集合 $\mathcal{B} = \{(i, j) \mid \text{关节 } i, j \text{ 相邻}\}$（H36M 17 关节默认 16 条骨）。对第 $k$ 条假设 $P_k$，计算其骨长向量 $\ell_k \in \mathbb{R}^{|\mathcal{B}|}$，并定义其与该视频骨长估计 $\hat \ell$（来自 Anatomy-Aware 的全视频 $l=50$ 帧聚合或本视频所有帧的中位数）的一致性分数：

$$
w_k = \exp\bigl(-\alpha \cdot \|\ell_k - \hat \ell\|_2^2 / |\mathcal{B}|\bigr) \in (0, 1]
$$

其中 $\alpha$ 是温度系数，在验证集上以 NLL 为目标调优。再加一项关节角限制项（仅膝肘二自由度，因为母题 4 指出"肩髋脊柱等复杂关节的约束从未被有效建模"）：
...
- **骨长估计**：可在 `common/load_data_hm36.py` 的 `Fusion` 类基础上新增一个 `BoneLengthEstimator`，从训练集 GT 3D 计算每 subject 的平均骨长，推理时按 subject ID 取用。
...
| 新增 `bone_length.py` | +40 | 解剖学重加权 | 低（新文件） |
