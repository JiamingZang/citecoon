...
> 技术可行性报告 · 2026-07-21 初版 / 2026-07-23 局部更新（RWKV-7 向量门控升级 + Test-time regression 框架） · idea: 惊讶度门控测试时记忆分离时序平滑与消歧.md · ReAct 写作（边写边查证 papers/cards/codebases）
...
2024–2025 年语言模型领域出现了"测试时记忆"（test-time memory）与"惊讶度门控"（surprise-gated memory）的新范式（Titans 系列，arXiv 预印本；cards/ 中无对应条目，具体数字待验证）。其核心思路是：不按固定窗口吞下全部上下文，而是按"惊讶度"（预测误差）选择性地把高信息量上下文写入压缩神经记忆，跳过冗余 token。Simple linear attention language models（arxiv:2402.18668）从 recall-throughput 权衡角度分析了线性注意力模型的选择性记忆能力，为惊讶度门控提供了理论参照。Gated Delta Networks（GDN，arxiv:2412.06464）实证表明门控（全局衰减/快速擦除）与 delta 规则（定向替换）在记忆管理上互补——单一机制无法同时覆盖"快速清除过时信息"与"精准替换特定键值对"两种需求，双门控递推 $S_t = \alpha_t S_{t-1}(I - \beta_t k_t k_t^\top) + \beta_t v_t k_t^\top$ 组合后性能超越 Mamba2 和 DeltaNet。Gated Linear Attention（GLA，arxiv:2312.06635）验证了数据依赖的对角门控（$G_t = \alpha_t^\top \mathbf{1}$）是标量门控与全秩门控之间的有效折中，兼顾表达力与硬件效率。GDN µP（arxiv:2606.04048）进一步推导出 GDN 的最大更新参数化规则：门控权重矩阵需 $\Theta(1/\sqrt{d})$ 学习率缩放，标量门控参数需 $\Theta(\sqrt{d})$ 缩放，为双门控模块的初始化与训练稳定性提供理论指导。RWKV-7（arxiv:2503.14456）进一步将标量门控推广为向量值逐通道学习率并解耦移除键 $\kappa$ 与替换键 $\tilde{k}$，使转移矩阵成为缩放 Householder 矩阵的近似（特征值稳定在 $[-1,1]$ 内），在 3.1T token 多语言语料上以 0.19B–2.9B 参数在多语言下游任务达到 3B SoTA、英语下游匹配 3B SoTA，为向量门控+解耦键设计提供了大规模实证。这一机制与姿态任务中"固定窗口收益递减"的瓶颈高度对应：视频中大量相邻帧是冗余的（仅贡献平滑），少数关键帧（运动突变、深度歧义解析点）才携带消歧信息。本工作将这一跨领域思路迁入 3D 姿态时序建模，并设计受控实验分离平滑、消歧、过拟合三种贡献。

**理论统一视角：Test-time Regression。** Test-time regression（arxiv:2501.12352）提出将关联回忆形式化为"记忆+检索"两步过程，其中记忆步骤被转化为对输入 token 的加权回归问题，在每次前向传播中重新求解回归器。该框架揭示任何序列层由三个正交设计选择唯一确定：（1）回归权重 $\gamma_i$（各历史帧的加权方案）；（2）回归器函数类 $\mathcal{M}$（参数化/非参数化）；（3）测试时优化算法（批量梯度下降、SGD、精确解等）。线性注意力、SSM、fast-weight 和 softmax attention 均为该框架的特例。惊讶度门控关联记忆可被纳入此统一框架：将记忆视为对历史帧的在线回归，其中回归权重 $\gamma_i$ 由惊讶度驱动（高惊讶帧获得更大权重），函数类 $\mathcal{M}$ 为低秩关联矩阵（$S_t \in \mathbb{R}^{d \times d}$），优化器为带遗忘的递推最小二乘（即双门控递推）。该框架为本方案的惊讶度加权设计提供了理论基础，同时暗示系统化的消融空间：不同权重方案（惊讶度驱动 vs 均匀 vs 可学习）和不同优化器（delta 规则 vs 简单衰减 vs 动量）可被逐一对照。
...
**从标量双门控到向量门控+解耦键（组件表达力升级）。** 上述标量双门控对所有记忆维度施加相同的衰减/写入率，表达力仍受限。RWKV-7（arxiv:2503.14456）将标量门控推广为向量值逐通道学习率并解耦移除键 $\kappa_t$ 与替换键 $\tilde{k}_t$，在保持线性复杂度的同时获得严格更强的表达力（特征值约束于 $[-1,1]$ 保证递推稳定性）。本工作据此将主方案进一步升级为 RWKV-7 式向量门控+解耦键递推（详见 §2.1.2），其中 $\boldsymbol{\alpha}_t \in (0,1)^d$、$\boldsymbol{\beta}_t \in (0,1)^d$ 为逐通道向量门控（每个记忆维度独立控制衰减/写入速率），$\kappa_t$ 为移除键（定位需擦除的旧信息），$\tilde{k}_t$ 为替换键（定位新写入信息的检索地址），二者解耦允许"擦除 A 位置、写入 B 位置"的灵活操作。对姿态任务而言，关节角度（慢变）与末端位置（快变）可拥有独立的记忆时间尺度。**标量双门控（$\alpha_t + \beta_t$）保留为消融基线**（见 §3.2 A14），用于量化向量化与键解耦的边际收益。
...
**向量门控+解耦键递推写入（主方案，RWKV-7 式）。** 记忆状态 $S_t \in \mathbb{R}^{d \times d}$ 按 RWKV-7 式向量门控递推更新（arxiv:2503.14456）：

$$S_t = \text{diag}(\boldsymbol{\alpha}_t) \, S_{t-1} \, (I - \boldsymbol{\beta}_t \, \kappa_t k_t^\top) + \boldsymbol{\beta}_t \, v_t \, \tilde{k}_t^\top$$

其中：
- $\boldsymbol{\alpha}_t \in (0,1)^d$ 为**逐通道衰减门控**（惊讶度驱动）：每个记忆维度独立控制衰减速率，高惊讶度帧 $\boldsymbol{\alpha}_t \to \mathbf{0}$，快速清空过时记忆；低惊讶度帧 $\boldsymbol{\alpha}_t \to \mathbf{1}$，保留已有信息；
- $\boldsymbol{\beta}_t \in (0,1)^d$ 为**逐通道写入强度**：控制对特定键值对的定向替换程度，各维度独立；
- $\kappa_t = W_\kappa f_t / \|W_\kappa f_t\|$（归一化移除键，定位需擦除的旧信息），$\tilde{k}_t = W_{\tilde{k}} f_t / \|W_{\tilde{k}} f_t\|$（归一化替换键，定位新写入信息的检索地址），$v_t = W_v f_t$。移除键与替换键解耦允许"擦除 A 位置、写入 B 位置"的灵活操作。

稳定性保证：RWKV-7 证明当 $\boldsymbol{\alpha}_t$ 各分量对应的特征值约束于 $[-1,1]$ 时递推严格稳定，通过参数化 $\alpha_{t,i} = -\exp(\xi_i)$（其中 $\xi_i \in [-5.3, 9.4]$）实现。$\boldsymbol{\alpha}_t$、$\boldsymbol{\beta}_t$ 均由当前帧特征经低秩线性层 + sigmoid 参数化，仅依赖当前输入以保持分块并行训练兼容性。初始化遵循 µP 缩放规则（arxiv:2606.04048）。

**标量双门控递推写入（消融基线，GDN 式）。** 将上述向量门控退化为标量（$\alpha_t, \beta_t \in (0,1)$，对所有通道施加相同衰减/写入率），并令 $\kappa_t = \tilde{k}_t = k_t$（移除键与替换键耦合），即得 GDN 式标量双门控递推（arxiv:2412.06464）：
...
**消融基线：单一阈值 $\tau$ 门控。** 原方案的写入决策为 $w_t = \sigma((s_t - \tau)/\beta)$，其中 $\tau$ 为可学习阈值，$\beta$ 为温度参数，$w_t \in [0,1]$ 为软写入权重。该方案作为三级门控对比的最低表达力基线保留（§3.2 A14：binary\_tau → dual\_scalar → dual\_vector\_decoupled），用于验证门控表达力梯度上的边际收益。

**记忆召回。** 主方案（向量门控+解耦键递推）与标量双门控消融基线中，召回均为矩阵-向量乘：$o_t = S_t \, q_t$（$q_t = W_q f_t$），无需显式 softmax 注意力，复杂度 $O(d^2)$。单一阈值 $\tau$ 消融基线中，记忆为显式 TopN key-value 存储（见下），召回为标准交叉注意力。
...
- **全局路径**：主方案中，每帧通过 RWKV-7 式向量门控+解耦键递推更新记忆状态 $S_t$，并以 $o_t = S_t q_t$ 召回长程消歧信息（标量双门控消融基线使用 GDN 式递推；单一阈值 $\tau$ 消融基线中，仅惊讶度超过阈值的帧写入 TopN 记忆，召回为交叉注意力）；
- **融合**：两路径输出以可学习门控权重融合：$\hat{y}_t = \gamma_t \cdot y_t^{\text{local}} + (1-\gamma_t) \cdot y_t^{\text{global}}$。

**伪代码（主方案：RWKV-7 式向量门控+解耦键递推）。**

```
输入: 2D 关键点序列 X ∈ R^{T×J×2}, DSTformer 骨干 B
初始化: 记忆状态 S_0 = 0 ∈ R^{d×d}, 可学习参数 W_α, b_α, W_β, b_β, W_κ, W_k̃, W_v, W_q, γ, ξ
...
for t = 1 to T:
    ...
    # 向量门控参数（由当前帧特征驱动，逐通道）
    α_t = sigmoid(W_α · f_t_local + b_α)   # ∈ (0,1)^d 逐通道衰减
    β_t = sigmoid(W_β · f_t_local + b_β)   # ∈ (0,1)^d 逐通道写入

    # 解耦键
    κ_t = normalize(W_κ · f_t_local)        # 移除键（定位需擦除的旧信息）
    k̃_t = normalize(W_k̃ · f_t_local)       # 替换键（定位新写入的检索地址）
    v_t = W_v · f_t_local

    # 向量门控+解耦键递推更新记忆状态
    S_t = diag(α_t) @ S_{t-1} @ (I - diag(β_t) @ κ_t @ k_t^T) + diag(β_t) @ v_t @ k̃_t^T
...
gate_type: "dual_vector_decoupled"  # "dual_vector_decoupled"（主方案，RWKV-7 式）| "dual_scalar"（消融基线，GDN 式）| "binary_tau"（消融基线）
weight_scheme: "surprise"  # "surprise"（惊讶度驱动）| "uniform" | "learned"（回归权重方案消融）
...
xi_init: "linear_interp"   # ξ 初始化策略：通道维度线性插值（慢变通道取大值、快变通道取小值）
xi_clip: [-5.3, 9.4]       # ξ 参数范围（依据 RWKV-7 实证统计）
xi_grad_clip: 1.0          # ξ 梯度裁剪（训练初期防止部分通道衰减过快/过慢）
# 标量双门控参数（消融基线，GDN 式）
scalar_alpha_init_bias: 0.0
scalar_beta_init_bias: 0.0
# 单一阈值参数（消融基线）
...
| A14 | **三级门控对比：binary\_tau → dual\_scalar → dual\_vector\_decoupled** | 验证门控表达力梯度上的边际收益（依据 GDN 互补性发现与 RWKV-7 向量化优势） | 向量门控+解耦键 MPJPE/MPJVE 应优于或持平标量双门控，标量双门控应优于或持平单阈值；result.json 以 `gate_type: "binary_tau"\|"dual_scalar"\|"dual_vector_decoupled"` 区分 |
| A15 | **回归权重方案消融：surprise vs uniform vs learned** | 验证惊讶度驱动加权的必要性（依据 Test-time regression 卡 arxiv:2501.12352 三设计轴） | 惊讶度驱动权重应优于均匀和可学习权重；result.json 以 `weight_scheme: "surprise"\|"uniform"\|"learned"` 区分 |
...
| 消融实验（A0–A15，共 16 组） | 1×RTX 3090（24 GB） | — | ~42 h 总计 | 含阈值扫描、容量扫描、三级门控对比和回归权重方案消融 |
...
| Titans 式惊讶度门控记忆的参考实现 | 低–中 | cards/ 中无 Titans 对应条目，但 GDN（arxiv:2412.06464，开源 github.com/NVlabs/GatedDeltaNet）、GLA（arxiv:2312.06635，开源 github.com/fla-org/flash-linear-attention）和 RWKV-7（arxiv:2503.14456，同仓库 fla/ops/rwkv7）提供了从标量双门控到向量门控+解耦键的完整参考实现 | 本工作的向量门控+解耦键机制直接借鉴 RWKV-7 递推公式，标量双门控借鉴 GDN，门控参数化借鉴 GLA，均有成熟开源代码可参照；µP 初始化参考 arxiv:2606.04048 |
...
| 向量门控 $\xi$ 参数超参敏感度（部分通道衰减过快/过慢） | 中 | 高（信息丢失或记忆污染） | $\xi$ 范围为 $[-5.3, 9.4]$（RWKV-7 实证统计，arxiv:2503.14456），跨度极大；初始化按通道维度线性插值（慢变通道取大值、快变通道取小值）；训练初期对 $\xi$ 施加梯度裁剪（clip=1.0）；监控各通道 $\alpha_{t,i}$ 分布，若出现双极分化则回退至标量双门控 |
...
- **惊讶度门控完全失效**：主方案中，若 $\xi$ 参数退化致 $\boldsymbol{\alpha}_t \equiv \mathbf{0}$（全衰减），记忆每步清零，全局路径无历史可召回，系统退化为纯局部路径；若 $\boldsymbol{\alpha}_t \equiv \mathbf{1}$（无衰减），记忆无限累积，系统退化为全帧信息的无差别压缩存储，预期略劣于基线（退化 ≤2 mm，待验证）。消融基线（binary\_tau）中，若门全开（$w_t \equiv 1$），所有帧写入记忆，系统退化为带固定大小记忆（$N$ 条）的交叉注意力增强 DSTformer；若门全关（$w_t \equiv 0$），系统退化为纯局部路径——即 MotionBERT 基线本身。
...
|------|----------|--------------------------|---------------------|------|
| 惊讶度计算 + 向量门控参数 | ~0（可学习低秩层 $W_\alpha, b_\alpha, W_\beta, b_\beta, \xi$） | 可忽略（$O(T \cdot J \cdot d)$ 线性层） | 可忽略 | 一阶差分 + 预测残差 → $\boldsymbol{\alpha}_t, \boldsymbol{\beta}_t$ |
| 向量门控+解耦键递推写入（$S_t$ 更新） | ~0（复用 $W_\kappa, W_{\tilde{k}}, W_v$） | 可忽略 | $O(d^2)$（矩阵-向量乘，$d=512$） | 主方案；消融基线为 TopN $O(1)$ 比较 |
| 记忆召回（$o_t = S_t q_t$） | ~2–3M（$W_q, W_\kappa, W_{\tilde{k}}, W_v$ 投影，5 层） | +17%（12 h → 14 h） | +5–15%（主方案 $O(d^2)$；消融基线 $O(J \cdot N \cdot d)$） | 召回为主要推理开销来源 |
...
| M5 | 消融实验（A0–A15） | 7 天 | 完整消融矩阵；阈值/容量敏感性分析；三级门控对比（binary\_tau→dual\_scalar→dual\_vector\_decoupled）；回归权重方案消融 | M4 |
...
本工作提出将 2024–2025 年语言模型领域的"惊讶度门控测试时记忆"迁入单目视频 3D 人体姿态估计的时序建模，以替代固定长度时序窗口。核心方案为：在 MotionBERT DSTformer（N=5, h=8, dim_feat=512, T=243）骨干的 Block 输出后、门控融合前（`lib/model/DSTformer.py:340-351`）插入 RWKV-7 式向量门控+解耦键惊讶度关联记忆模块——以逐通道衰减门控 $\boldsymbol{\alpha}_t$（控制各维度遗忘速率）与逐通道写入 $\boldsymbol{\beta}_t$（控制定向替换强度）的递推 $S_t = \text{diag}(\boldsymbol{\alpha}_t) S_{t-1}(I - \boldsymbol{\beta}_t \kappa_t k_t^\top) + \boldsymbol{\beta}_t v_t \tilde{k}_t^\top$ 替代固定窗口，移除键 $\kappa_t$ 与替换键 $\tilde{k}_t$ 解耦允许灵活的擦除-写入操作，使记忆管理自适应于运动复杂度，在不增加序列长度的情况下收获长程消歧信息、跳过冗余帧（标量双门控 GDN 式与原单一阈值 $\tau$ 方案保留为消融基线）。理论上，惊讶度门控关联记忆可被纳入 Test-time regression（arxiv:2501.12352）统一框架——将记忆视为对历史帧的在线回归，回归权重由惊讶度驱动，三设计轴（权重/函数类/优化器）提供系统化消融空间。配套设计平滑-消歧-过拟合三分解诊断协议（MPJVE 衡量平滑、深度翻转率 DFR 衡量消歧、慢/快留出动作误差差值 SFG 衡量过拟合），首次定量回答"固定窗口 243 帧的收益中，平滑、消歧、过拟合各占多少"这一领域根源问题。预期收益：MPJPE 改善 0.4–1 mm（检测 2D 设置，H3.6M），同时提供首个受控三分解实验框架。主要风险：向量门控 $\xi$ 参数超参敏感度（范围 $[-5.3, 9.4]$，需通道维度线性插值初始化与梯度裁剪）、双门控初始化需遵循 µP 缩放规则以维持训练稳定性、DFR 指标定义合理性需验证、精度改善可能不足以单独支撑顶会主会。时间框架约 5 周（单卡 RTX 3090 可闭环，总 GPU 成本 ~$36），目标会议为 ECCV/CVPR Workshop（诊断优先路径）或 ICCV/ECCV 主会（精度优先路径，需扩展至 8–10 周）。
...
| RWKV-7 | RWKV-7 "Goose" with Expressive Dynamic State Evolution | arxiv:2503.14456 | 2025 | 0 |
| Test-time Regression | Test-time regression: a unifying framework for designing sequence models with associative memory | arxiv:2501.12352 | 2025 | 0 |
