...
## Gap 来源（结构依据）
母题「视觉基础模型（DINOv2/MASt3R/SAM）被默认为'即插即用的通用几何-语义特征源'，但其跨域、跨尺度、跨模态的泛化边界从未被系统界定」张力：所有方法假设基础模型中间层特征天然编码了足够的3D几何与视角信息，但DINOv2/CLIP的训练目标是语义判别而非几何度量——特征空间中的'相似'是否等价于'几何对应'从未被验证。当物体无纹理/对称时，语义特征丧失判别力（**ZS6D在T-LESS上的系统性失败直接证实了这一点**），但无人提出特征可靠性的在线检测机制。此外，选择哪一层（第9层vs第18层）完全靠经验，缺乏理论指导。**新母题进一步指出：'冻结特征足够好'的隐含假设使整个领域回避了'何时该微调、微调多少'的核心问题——本idea的自适应层选择+可靠性门控+MASt3R回退正是对该问题的零重训练回答：不微调，但系统性地界定冻结特征的适用边界并在越界时切换特征源。**
...
## 动机
DINOv2已成为未见物体位姿估计的事实特征提取器，但不同工作在没有理论依据的情况下各自选择不同层（FoundPose用第18层，ZS6D用ViT-S/8全层聚合）。当物体属性（低纹理、对称、反光）或光照条件发生变化时，固定层选择可能导致性能大幅退化。**ZS6D在T-LESS（无纹理工业零件）上的失败模式表明，DINOv2语义特征在特定物体属性下确实丧失判别力，但失败被笼统归因于"方法局限"而非特征层选择或特征可靠性问题。** 新母题明确指出：DINOv2在ImageNet自然图像上预训练，**对工业无纹理零件、CAD渲染图的适用性仅靠少量实验间接验证**（来自母题#3共享假设张力点），而非系统性界定。理解何时DINOv2特征失效、哪些层在哪些条件下最优、以及何时应切换到几何训练特征（如MASt3R），是提升整个model-free位姿估计系统鲁棒性的基础性问题。目前这一问题完全空白，解决它能直接指导FoundPose、GS-Pose、Cross-View Semantic Priors、SAM-6D等多个下游方法的特征选择设计，并回应母题中明确指出的"无人提出特征可靠性的在线检测机制"与"何时该微调、微调多少"两个开放问题。
...
## 技术路线
1）**层扫描与属性热力图**：利用FoundPose的卡片方法，在BOP的7个测试数据集上，对FoundPose流水线做层扫描实验（层6/12/18/24/30/36，ViT-L/14），记录每层在不同物体类型（按纹理密度、对称性、材质分组）下的AR分数，绘制'物体属性×层深度→AR'热力图。2）**MASt3R对照实验**：在相同物体分组上，用MASt3R的24维局部描述子（经InfoNCE几何对应训练）替换DINOv2特征执行相同流水线，记录AR分数，对比两者的'属性敏感性曲线'——若MASt3R在低纹理/对称组上退化显著更小，则证实DINOv2层敏感性的根源是语义训练目标而非架构本身。**分辨率边界条件（来自DUSt3R评估卡 arxiv:2507.14798）**：MASt3R/DUSt3R架构将输入缩放至最大边518像素，高分辨率场景（如航摄影像）精度骤降；BOP数据集图像分辨率约640×480，接近518px限制，故本对照实验在BOP上有效且信息损失有限。但若将结论推广至高分辨率工业检测场景（如4K+），需额外考虑MASt3R的分辨率瓶颈——此时DINOv2（原生支持更高分辨率patch化）可能反而具有架构优势，自适应策略应纳入分辨率作为额外决策维度。3）**属性分类器与自适应选择**：发现物体属性与最优层的映射规律后，设计一个轻量级属性分类器（基于图像patch统计量：HOG梯度方差衡量纹理、Hu矩衡量对称性、反光度用高亮像素比例近似），在推理时动态选择最优DINOv2层；**同时输出特征可靠性分数**（基于所选层patch描述子的余弦相似度分布熵），当可靠性低于阈值时标记为"特征不可信"并触发MASt3R回退路径。4）将自适应层选择+可靠性门控插入FoundPose流水线（零重训练），在BOP标准评测上验证提升。参考Gen6D的特征体积设计思路、On Continuity of Rotation Representations的理论分析框架、以及Circle Loss中自适应重加权的思想（偏离最优越远惩罚越大→类比：属性偏离训练分布越远，层选择越关键）。
...
- arxiv:2507.14798 — An Evaluation of DUSt3R/MASt3R/VGGT 3D Reconstruction on Photogrammetric Aerial Blocks（MASt3R 518px分辨率瓶颈的实证来源）
...
- **2026-07-23 · 局部更新（强化+边界条件补充）**：
  - 判定类型：局部更新（母题重算，无新卡；核心信息流未受挑战）
  - Gap来源更新：母题引用升级为新版表述——"视觉基础模型被默认为即插即用的通用几何-语义特征源，但其跨域、跨尺度、跨模态的泛化边界从未被系统界定"；补入"'冻结特征足够好'的假设使整个领域回避了'何时该微调、微调多少'的核心问题"这一张力点，并明确本idea作为该问题的零重训练回答之定位。
  - 动机强化：补入新母题中"DINOv2对工业无纹理零件、CAD渲染图的适用性仅靠少量实验间接验证"的明确表述（来自母题#3共享假设张力点）。
  - 技术路线第2步增加MASt3R分辨率边界条件注释（来自DUSt3R评估卡 arxiv:2507.14798）：MASt3R将输入缩放至518px，高分辨率场景精度骤降；BOP数据（~640×480）接近该限制故对照有效，但推广至高分辨率工业场景需纳入分辨率作为额外决策维度。
  - 相关论文新增 arxiv:2507.14798 条目。
  - 未做结构性重构，因为：本轮变化仅强化动机定位并为MASt3R对照补充边界条件，"层扫描→属性映射→自适应选择→可靠性门控→MASt3R回退"的核心信息流与模块划分完全不受影响。
