...
注：在线阶段 MASt3R 对应可通过模板预筛选（取 top-5 而非 60）压缩至 ~1 s；特征精修为主要耗时，优化后预期（见 §4.4）。耗时以标准 MASt3R（198 ms/对）为基准，Speedy MASt3R（91 ms/对）作为加速选项。
...
**对吞吐的影响：** 优化后 ~14 s/图 ≈ 0.07 FPS，不满足实时需求但满足离线批处理（10K 测试图 ≈ 39 小时）。iG-6DoF 报告 0.5 s/图（含 0.4 s 精修），但其精修为单尺度 SSIM（无 ViT 前向），本方案的额外耗时来自 ViT 编码器前向/反向——这是用基础模型特征替代光度损失的固有成本。
