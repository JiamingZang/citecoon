...
**3D 基础模型先验的泛化边界。** 然而，3D 几何基础模型的先验层级并非无条件适用。MASt3R/DUSt3R 的输入图像被缩放至最大边 518 像素（arxiv:2507.14798，DUSt3R/MASt3R/VGGT 航摄评估），超出此分辨率的输入须降采样，精度随之下降；同时，点图回归范式输出 up-to-scale 重建，缺乏绝对尺度约束——在 191 图规模的航测场景中 VGGT 位姿漂移达 42 m（arxiv:2507.14798），表明尺度不确定性是该范式的未解结构性缺陷。这两项限制共同构成 3D-FM 先验层级的适用域边界：在 BOP 标准 640×480 分辨率、室内物体尺度（5-30 cm 直径）条件下，3D-FM 先验有效（Pos3R 粗估计 AR 39.5 即为正面证据）；但在高分辨率工业检测或度量级大尺度户外场景中，Pareto 前沿的形状可能显著改变，不可无条件外推。本工作在 §2.2 敏感性分析中对此做方法学准备。
